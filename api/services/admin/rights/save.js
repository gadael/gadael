...
function saveRight(service, params) {
    ...
    if (params.id)
    {
        RightModel.findByIdAndUpdate(params.id, fieldsToSet, function(err, document) {
            if (service.handleMongoError(err))
            {
                service.resolveSuccess(
                    calendar, 
                    document.gt.gettext('The vacation right has been modified')
                );
            }
        });
        ...
        RightModel.create(fieldsToSet, function(err, calendar) {

            if (service.handleMongoError(err))
            {
                service.resolveSuccess(
                    calendar, 
                    document.gt.gettext('The vacation right has been created')
                );
            }
        });
    }
